...
router.get('/',(req, res) => {
    Post.find()
        ...
        .catch(err => res.status(404).json({noPost: "No posts are found with that id"}));
...
//Description   Delete comment to the post
//@access       Private
router.delete('/comment/:id/:comment_id', passport.authenticate('jwt', {session: false}), (req, res) => {
    Post.findById(req.params.id)
        ...
        //    Make sure only comment owner delete comment
            ...
            //Save the comments in the post now
            post.save().then(post => res.json(post));
